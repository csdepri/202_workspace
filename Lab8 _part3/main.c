...
//    function. Pressing pb2 reads the value on channel 5 of the adc0 which
//    is connected to the thermistor. It then converts the value to farhenheit 
//    and disaplays it to the lcd. This diplay does not change until pb2 is 
//    pressed again and a new value is displayed. 
//
// INPUT PARAMETERS:
//  none
//
...
void run_lab8_part3 (void)
{
    ...
    //write static message to lcd
    lcd_set_ddram_addr(LCD_LINE1_ADDR);
    lcd_write_string("ADC = ");
    lcd_set_ddram_addr(LCD_LINE2_ADDR);
    lcd_write_string("Temp = xxF");

    bool done_flag = false;
    
    while(!done_flag)
    {
        // disable interupts then read then enable interupts so that pb2 does
        // not change adc config furing reading.
        NVIC_DisableIRQ(GPIOA_INT_IRQn);
        uint16_t adc_in = ADC0_in(7);
        NVIC_EnableIRQ(GPIOB_INT_IRQn);
        
        //divide adc input into a valuye within the range of 9
        ...
        uint16_t temp;
        //if pb2 is pressed read channel 5 of adc and convert to farenheit
        if(g_PB2_pressed)
        {
            temp = thermistor_calc_temperature(ADC0_in(5));
            lcd_set_ddram_addr(LCD_LINE2_ADDR | LCD_CHAR_POSITION_7);
            lcd_write_doublebyte(((temp * 1.8)+32));
            g_PB2_pressed = false;
        }
